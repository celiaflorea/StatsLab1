...
observations <- rnorm(5*25, 200, 100)
samples <- rep(1:5, each = 25)
my_data <- data_frame(samples, observations)
...
sd_N <- function(x) sqrt(sum((mean(x)-x)^2) / length(x))
...
```
#note: getting an error on 4: "summarise()` ungrouping output (override with `.groups` argument)" but it seems to be running fine and it doens't point to a specific line that needs fixing so I'm leaving as is. Making a note to ask about this next lab but seems to be processing ok. 
